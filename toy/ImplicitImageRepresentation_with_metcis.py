...
def sine_cosine_embedding(num_basis):
    def f(x):
        ...
        s = torch.cat([torch.sin((2**i)*x*torch.pi)
                      for i in range(num_basis//2)], dim=1)
        c = torch.cat([torch.cos((2**i)*x*torch.pi)
                      for i in range(num_basis//2)], dim=1)
...
def get_imgs(image_paths):
    ...
    l = [T(Image.open(path).convert('RGB')).float()/255. for path in image_paths]
...
def main():
    ...
    models = [
        ...
        MLP(nn.LeakyReLU,    layers, name=f'LeakyReLU pe sin+cos',embedding_fn=sine_cosine_embedding(num_basis)).cuda(),
    ]
    ...
    fig.tight_layout()
    [ma.grid(True, which="both", ls='-', alpha=.3) for ma in metric_ax]
    ...
    while(True):
        for idx, (model, optims, im_ax) in enumerate(zip(models, optimizers, im_axes)):
            ...
            structural_sim = ssim_loss(rgb_gt.reshape(h,w,c).cpu().permute(2,0,1).unsqueeze(0), reco_image.permute(2,0,1).unsqueeze(0), window_size=7)
            ...
            loss_plot[idx].set_data(lines[model.name]['epoch'], lines[model.name]['loss'])
            psnr_plot[idx].set_data(lines[model.name]['epoch'], lines[model.name]['psnr'])
            ssim_plot[idx].set_data(lines[model.name]['epoch'], lines[model.name]['ssim'])
